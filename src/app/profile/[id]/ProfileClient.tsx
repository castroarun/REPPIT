...
import { StrengthCard, LevelLegend, BodyPartFilter, FilterMode } from '@/components/strength'
import { getSelectedRoutine, getSelectedExercises, WorkoutRoutine } from '@/lib/storage/routines'
import { MotivationalQuote } from '@/components/quotes'
import { ContextualTip, hasSeenTip } from '@/components/onboarding'
...
export default function ProfileDetailPage({ params }: ProfileDetailPageProps) {
  ...
  const [settingsTipSeen, setSettingsTipSeen] = useState(false)
  // Filter mode state
  const [filterMode, setFilterMode] = useState<FilterMode>('bodyPart')
  ...
  useEffect(() => {
    ...
    // Check if settings tip was already seen (for page reloads)
    setSettingsTipSeen(hasSeenTip('profile_settings'))

    // Poll for tip state changes (when user returns from settings after re-enabling tips)
    const tipIntervalId = setInterval(() => {
      if (areTipsEnabled()) {
        // If tips were reset, update settingsTipSeen to reflect current state
        setSettingsTipSeen(hasSeenTip('profile_settings'))
      }
    }, 1000)

    // Load selected routine for program mode
    const routine = getSelectedRoutine()
    setSelectedRoutine(routine)
    if (routine) {
      setProgramExercises(getSelectedExercises())
    }
  ...
  // Get exercises for the current selected workout day when in program mode
  const currentDayExercises: Exercise[] = filterMode === 'program' && selectedRoutine
    ? selectedRoutine.days[selectedDayIndex]?.exercises.filter(ex => programExercises.includes(ex)) || []
    : []

  // Filter exercises based on mode
  let ratedExercises = getRatedExercises(profile.weight, validatedRatings, bodyPartFilter, profile.sex)
  let unratedExercises = getUnratedExercises(profile.weight, validatedRatings, bodyPartFilter, profile.sex)

  // If in program mode, filter to only show exercises from the selected day
  if (filterMode === 'program' && selectedRoutine) {
    ratedExercises = ratedExercises.filter(ex => currentDayExercises.includes(ex.exercise))
    unratedExercises = unratedExercises.filter(ex => currentDayExercises.includes(ex.exercise))
  }
  ...
  return (
    <div className="min-h-screen pb-20">
      ...
      <main className="p-4 max-w-lg mx-auto">
        ...
        {/* Exercise Filter */}
        <div className="mb-4">
          <p className="text-xs text-gray-500 dark:text-gray-400 mb-2">Filter exercises:</p>
          <BodyPartFilter
            selected={bodyPartFilter}
            onChange={setBodyPartFilter}
            mode={filterMode}
            onModeChange={setFilterMode}
            workoutDays={selectedRoutine?.days || []}
            selectedDayIndex={selectedDayIndex}
            onDayChange={setSelectedDayIndex}
            hasProgram={!!selectedRoutine}
          />
        </div>
      ...
      {/* Contextual Tip - Settings access */}
      <ContextualTip
        tipId="profile_settings"
        title="Customize Your Experience"
        message="Tap the ⚙️ gear icon to access Settings. Configure tips, timer defaults, training program, and more."
        icon="⚙️"
        position="top"
        onDismiss={() => setSettingsTipSeen(true)}
      />

      {/* Contextual Tip - Tap to expand */}
      <ContextualTip
        tipId="profile_tap_exercise"
        title="Tap to Log Workout"
        message="Tap any exercise card to expand it and log your sets. Your workout history and PRs are tracked automatically!"
        icon="👆"
        position="bottom"
      />

      {/* Contextual Tip - Set up training program (only when no program selected) */}
      {!selectedRoutine && (
        <ContextualTip
          tipId="training_program_setup"
          title="Set Up Your Training Program"
          message="Go to Settings → Training Program to choose your workout split (Push/Pull/Legs, Upper/Lower, etc.) and filter exercises by workout day!"
          icon="📋"
          position="top"
        />
      )}
